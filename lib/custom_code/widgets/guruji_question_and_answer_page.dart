...
class GurujiQuestionAndAnswerPage extends StatefulWidget {
  final DocumentReference questionRef;
  final double? width;
  final double? height;

  const GurujiQuestionAndAnswerPage({
    Key? key,
    this.width,
    this.height,
  ...
  @override
  _GurujiQuestionAndAnswerPageState createState() =>
      _GurujiQuestionAndAnswerPageState();
}

class _GurujiQuestionAndAnswerPageState
    extends State<GurujiQuestionAndAnswerPage> {
  ...
  Future<void> fetchAnswers() async {
    final snapshot = await currentQuestionRef.collection('guruji_answer').get();
  ...
  Future<void> checkFavorite() async {
    final favSnapshot = await FirebaseFirestore.instance
        .collection('guruji_question_favourite')
  ...
  Future<void> checkBookmark() async {
    final bookmarkSnapshot = await FirebaseFirestore.instance
        .collection('guruji_question_bookmark')
        .where('questionDocumentID', isEqualTo: currentQuestionRef.id)
        .where('uid', isEqualTo: currentUser?.uid)
        .limit(1)
        .get();
        ...
        SizedBox(
          ...
          child: Stack(
            ...
            children: [
              ...
              const Text(
                'गुरुदेव द्वारा प्रश्नों के ज्ञानवर्धक उत्तर',
                style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.bold,
                    color: Color(0xFFC25123)),
          ...
          children: [
            ...
            IconButton(
              icon: Icon(Icons.favorite,
                  size: 30, color: isFavorite ? Colors.red : Colors.black),
              onPressed: toggleFavorite,
            ),
            IconButton(
              icon: Icon(Icons.stars,
                  size: 30, color: isBookmark ? Colors.orange : Colors.black),
